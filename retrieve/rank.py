    ...
    with tqdm(total=len(result)) as pbar:
        for k in range(K):
            ...
            for _id, item in enumerate(result):
                ...
                top_K_text_ids = [text_id for text_id in text_ids[:k+1]]

                if true_docid in top_K_text_ids:
                    correct[k] += 1
...
def build_train_data(train_data, k=5):
    save_path = '../data/rank/train_data_top{}.pkl'.format(k)
    train_datas = []
    count = 0
    with tqdm(total=len(train_data)) as pbar:

        pbar.set_description('building train data:')
        for _id, item in enumerate(train_data):
            true_docid = item['docid']
            text_ids = item['text_ids']
            top_K_text_ids = [text_id for text_id in text_ids[:k]]

            if true_docid not in top_K_text_ids:
                top_K_text_ids.pop()
                top_K_text_ids.insert(0, true_docid)
                count += 1
            ...
            train_datas.append(train_sample)
            pbar.update(1)
    print("{} example without true docid".format(count))
    with open(save_path, 'wb') as fp:
        pickle.dump(train_datas, fp)
...
if __name__ == '__main__':
    ...
    path = {
        ...
        'train_dict': '../data/rank/train_dict2.pkl',
        'context_dict': './data/rank/context_dict2.pkl',
        'context_corpus': '../data/rank/context_corpus2.pkl',
        'result': '../data/rank/result_bm25.pkl',
        'bad_samples': '../data/rank/bad_samples2.pkl',
        'context_list': '../data/rank/context_list2.pkl',
        'context_idmap': '../data/rank/context_idmap2.pkl',
        'BM25_model': '../data/rank/BM25_model2.gen'
    }
    ...
    if os.path.exists(path.get('result')) and True:
        ...
        build_train_data(train_data, k=5)
    # cal_MAP(result)
