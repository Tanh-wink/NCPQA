...
class Data_processor1(object):
        ...
        with tqdm(total=len(examples), desc="convert examples to features:") as pbar:
            for example_id, example in enumerate(examples):
                ...
                for context_index, (docid, context) in enumerate(zip(docids, contexts)):
                    ...
                    q_tokens = ["[CLS]"] + qusetion_tokens + ["[SEP]"]
                    q_input_mask = [1] * len(q_tokens)
                    padding = ["[PAD]"] * (self.max_query_length + 2 - len(q_tokens))
                    q_tokens = q_tokens + padding
                    q_input_mask += [0] * len(padding)
                    q_segment_ids = [0] * len(q_tokens)
                    for (doc_span_index, doc_span) in enumerate(doc_spans):
                        ...
                        for i in range(doc_span.length):
                            ...
                            token_to_orig_map[len(docs_tokens)] = token_to_orig_index[split_token_index]

                            is_max_context = self._check_is_max_context(doc_spans, doc_span_index,
                                                                   split_token_index)
                            token_is_max_context[len(docs_tokens)] = is_max_context
                            docs_tokens.append(context_tokens[split_token_index])
                        docs_tokens.append("[SEP]")
                        docs_input_mask = [1] * len(docs_tokens)
                        padding = ["[PAD]"] * (max_context_length + 1 - len(docs_tokens))
                        docs_tokens = docs_tokens + padding
                        docs_input_mask += [0] * len(padding)
                        tokens = q_tokens + docs_tokens
                        doc_segment_ids = [1] * (len(docs_tokens))

                        input_ids = self.tokenizer.convert_tokens_to_ids(tokens)
                        input_mask = q_input_mask + docs_input_mask
                        segment_ids = q_segment_ids + doc_segment_ids
    ...
    def prepare_train_dataloader(self,  train_features, train_batch_size, local_rank):
        ...
        train_data = TensorDataset(all_unique_ids, all_input_ids, all_input_mask, all_segment_ids,
                                   all_start_idxs, all_end_idxs)
        if local_rank == -1:
