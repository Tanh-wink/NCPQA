...
class InputFeatures(object):
    ...
    def __init__(self, unique_id, qid, context_index, doc_span_index, token_to_orig_map, token_is_max_context, answer,
                 tokens, input_ids, input_mask, segment_ids, docid, start_idx, end_idx, label):
...
class Data_processor(object):
        ...
        with tqdm(total=len(data_set), desc="reading examples:") as pbar:
            for index, data in enumerate(data_set):
                ...
                if is_train:
                    docid = data['docid']
                    docids.append(docid)
                    contexts.append(self.id2doc[docid])
        ...
        with tqdm(total=len(examples), desc="convert examples to features:") as pbar:
            for example_id, example in enumerate(examples):
                ...
                for context_index, (docid, context) in enumerate(zip(docids, contexts)):
                    ...
                    title_tokens = self.tokenizer.tokenize(title) if len(title) > 0 else []
                    if len(title_tokens) > self.max_query_length:  # cut at tail
                        title_tokens = title_tokens[0:self.max_query_length]
                    ...
                    max_context_length = self.max_seq_length - self.max_query_length * 2 - 4
                    ...
                    for (doc_span_index, doc_span) in enumerate(doc_spans):
                        ...
                        tokens += title_tokens + ["[SEP]"]
                        segment_ids += [1] * len(title_tokens + ["[SEP]"])
    ...
    def get_train_features(self, train_examples, train_features_file, doc_stride):
        ...
        if os.path.exists(train_features_file):
            logger.info('load train feature cache: {}'.format(train_features_file))
    ...
    def prepare_pred_dataloader(self, predict_file, predict_batch_size, doc_stride, cache_dir=None):
        ...
        batch_n = 200
        for batch_start in np.arange(0, example_n, batch_n):
            ...
            if os.path.exists(cache_file):
                ...
                t1 = time.time()
                batch_indices = indices[batch_start: batch_start + batch_n]
                example_batch = []
