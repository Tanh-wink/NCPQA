...
class QAExample(object):
    def __init__(self, qid=None, qusetion=None, docids=None, contexts=None, answer=None, starts=None, ends=None):
...
class Data_processor_multi(object):
    ...
    def _convert_examples_to_features(self, examples, doc_stride, is_train=True):
                ...
                for context_index, (docid, context) in enumerate(zip(docids, contexts)):
                    ...
                    if is_train:
                        for start, end in zip(starts, ends):
                            ...
                            if end < len(context) - 1:
                                token_end_position = orig_to_token_index[end + 1] - 1
                            else:
                            ...
                            token_start_positions.append(token_start_position)
                            token_end_positions.append(token_end_position)
                    ...
                    for (doc_span_index, doc_span) in enumerate(doc_spans):
                        ...
                        if is_train:
                            ...
                            tok_end_position = -1
                            for token_start_position, token_end_position in zip(token_start_positions, token_end_positions):
                                if not (token_start_position >= doc_start and
                                        token_end_position <= doc_end):
                                    out_of_span = True
                                    tok_start_position = token_start_position
                                    tok_end_position = token_end_position
                                    break
                            if not out_of_span:
        ...
        print('true_count {}, bad_count {}'.format(true_count, bad_count))
        return features
            # for example_id, example in enumerate(examples):
            #     qid = example.qid
            #     qusetion = example.qusetion
            ...
            #         if is_train:
            #             for start in starts:
            #                 token_start_positions.append(orig_to_token_index[start])
            #             for end in ends:
            #                 if end < len(context) - 1:
            #                     token_end_positions.append(orig_to_token_index[end+1] - 1)
            #                 else:
            #                     token_end_positions.append(len(context_tokens) - 1)
            #
            #         # We can have documents that are longer than the maximum sequence length.
            #         # To deal with this we do a sliding window approach, where we take chunks
            #         # of the up to our max length with a stride of `doc_stride`.
    ...
    def _convert_examples_to_features_sub_doc(self, examples):
        """Loads a data file into a list of `InputBatch`s."""
        unique_id = 1000000000
        features = []
        total_count = 0
    ...
    def prepare_pred_dataloader(self, predict_file, predict_batch_size, doc_stride, cache_dir=None):
        ...
        for batch_start in np.arange(0, example_n, 400):
            cache_file = os.path.join(cache_dir, 'pred_features_multi_{}.pkl'.format(batch_start))
    ...
    def prepare_pred_dataloader_sub_docs(self, predict_file, predict_batch_size, cache_dir=None):
        pred_examples = self.read_QA_examples_sub_doc(
            predict_file)
        # eval_examples=eval_examples[:100]
        ...
        for batch_start in np.arange(0, example_n, 400):
            ...
            if os.path.exists(cache_file):
                ...
                for batch_indice in batch_indices:
                    example_batch.append(pred_examples[batch_indice])
                pred_features = self._convert_examples_to_features_sub_doc(
                    examples=example_batch,
                )
                with open(cache_file, 'wb') as fp:
                    fp.write(pickle.dumps(pred_features))
