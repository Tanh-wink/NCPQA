...
from prepare.prepare_data_rank import Data_processor
import time
...
def main():
    ...
    if args.do_train:
        ...
        model = train(args, model, train_dataloader, device, num_train_steps,
                      valid_examples=valid_examples, valid_features=valid_features, valid_dataloader=valid_dataloader, n_gpu=n_gpu, train_writer=train_writer, valid_writer=valid_writer, tokenizer=tokenizer)
...
def train(args, model, train_dataloader, device, num_train_steps,
          valid_examples=None, valid_features=None, valid_dataloader=None, n_gpu=1, train_writer=None, valid_writer=None, tokenizer=None):
    '''
        ...
        with tqdm(total=len(train_dataloader)) as pbar:
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) % args.valid_step == 0:
                    if args.do_valid:
                        if valid_writer:
                            Rouge_L, valid_loss, P, R = valid(args, model, valid_examples, valid_features, valid_dataloader, device, valid_writer=valid_writer, global_step=global_step, tokenizer=tokenizer)
                            logger.info(
...
def valid(args, model, valid_examples, valid_features, valid_dataloader, device, valid_writer=None, global_step=None, tokenizer=None):
    '''
...
def write_predictions(args, all_examples, all_features, all_results, n_best_size=1, output_prediction_file=None, tokenizer=None):
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    for pred in prelim_predictions:
      ...
      if pred.start_index > 0:  # this is a non-null prediction
        ...
        final_text = get_final_text(tok_text, orig_text, args.do_lower_case, args.verbose_logging, tokenizer=tokenizer)
